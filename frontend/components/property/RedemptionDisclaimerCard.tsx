...
import { redemptionRules, redemptionDefinitions, RedemptionRule } from '../../data/redemptionData';
...
export const RedemptionDisclaimerCard: React.FC<RedemptionDisclaimerCardProps> = ({ state, auctionType }) => {
    ...
    // Matching logic
    // 1. Exact match on state & auctionType
    ...
    // 2. Fallback to state match only if no exact match found
    if (matchedRules.length === 0) {
        matchedRules = redemptionRules.filter(r => r.state.toLowerCase() === state.toLowerCase());
    }
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                            Jurisdiction: {state} {auctionType ? `| ${auctionType}` : ''}
                        </p>
                    </div>
                </div>
            ...
            {expanded && (
                <div className="p-6">
                                            ...
                                            <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                                ...
                                                <td className="px-4 py-3">
                                                    <span className={`px-2 py-0.5 rounded text-xs font-bold ${
                                                        rule.type === 'Deed' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400' :
                    ...
                    <div className="mt-4 p-4 bg-slate-50 dark:bg-slate-900/50 border-l-4 border-slate-300 dark:border-slate-600 rounded-r-lg text-xs text-slate-500 dark:text-slate-400 italic leading-relaxed">
                        <strong>Disclaimer:</strong> As informações acima são de caráter educativo e baseadas em dados gerais por estado. Regras podem variar por county e caso específico. Consulte sempre um advogado especializado em direito imobiliário e fiscal antes de realizar qualquer investimento.
                    </div>
                </div>
            )}
        </div>
