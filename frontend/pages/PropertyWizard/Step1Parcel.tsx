...
export const Step1Parcel: React.FC<Props> = ({ data, update }) => {
    ...
    return (
        <div className="p-8">
            <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-lg font-bold text-slate-900 dark:text-white">Parcel & Location</h2>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-6">
                    ...
                    <div ref={wrapperRef} className="relative z-[1000]">
                        ...
                        {showSuggestions && suggestions.length > 0 && (
                            <ul className="absolute left-0 right-0 mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-xl max-h-60 overflow-y-auto z-[1001]">
                                {suggestions.map((s, i) => (
                                    <li key={s.id || i} className="px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-700 cursor-pointer border-b border-slate-100 last:border-0" onClick={() => handleSelectAddress(s)}>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Parcel ID (APN)</label>
                            <input
                                ...
                                onChange={(e) => {
                                    ...
                                    update({ parcel_id: newParcelId, smart_tag: smartTag.toUpperCase() });
                                }}
                            />
                        </div>
                        <div>
                    ...
                    <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700">
                        <div className="flex justify-between items-center mb-3">
                            ...
                            <button type="button" onClick={() => setShowMapSelector(true)} className="text-xs text-blue-600 font-medium">Select from Map</button>
                        </div>
                        {showMapSelector && (
                            <CountySelector
                                mode="select"
                                ...
                                onSelect={(state, county) => {
                                    const smartTag = [state, county, data.parcel_id].filter(Boolean).join('-').toUpperCase();
                                    update({ state, county, smart_tag: smartTag });
                                    setShowMapSelector(false);
                                }}
                            />
                        )}
                        <div className="grid grid-cols-2 gap-4">
                            <div><label className="text-xs text-slate-500">City</label><input className="w-full rounded text-xs border-slate-300" value={data.city || ''} onChange={e => update({ city: e.target.value })} /></div>
                            <div><label className="text-xs text-slate-500">State</label><input className="w-full rounded text-xs border-slate-300" value={data.state || ''} onChange={e => update({ state: e.target.value })} /></div>
                            <div><label className="text-xs text-slate-500">County</label><input className="w-full rounded text-xs border-slate-300" value={data.county || ''} onChange={e => update({ county: e.target.value })} /></div>
                            <div><label className="text-xs text-slate-500">Zip</label><input className="w-full rounded text-xs border-slate-300" value={data.zip_code || ''} onChange={e => update({ zip_code: e.target.value })} /></div>
                        </div>
                    </div>
                </div>

                {/* Map Preview */}
                <div className="h-64 md:h-full w-full rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700 bg-slate-50">
                    <LocationPickerMap
                        ...
                        onLocationSelect={(locData) => {
                            update({
                                ...
                                smart_tag: [locData.state || data.state, locData.county || data.county, data.parcel_id].filter(Boolean).join('-').toUpperCase()
                            });
